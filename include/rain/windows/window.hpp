...
namespace Rain::Windows {
	class Window {
		public:
		// Aggregate class for designated initialization
		// (C++20).
		class CreateParameters {
			public:
			// A one-time class name will be generated if this is
			// kept NULL. Otherwise, no call to RegisterClassEx
			// will be made, and an existing window class will be
			// used instead.
			...
			HBRUSH hBrBackground{
				reinterpret_cast<HBRUSH>(COLOR_WINDOWFRAME)};
		...
		static LRESULT CALLBACK windowProcedure(
			...
			LPARAM lParam) {
			if (uMsg == WM_CREATE) {
				// When window is created, set its long pointer to
				// the object.
		...
		static HWND createWindow(
			...
			// className must be kept in scope for the duration of
			// the function.
		...
		virtual LRESULT onContextMenu(
			...
			return DefWindowProc(
				hWnd, WM_CONTEXTMENU, wParam, lParam);
		}
		...
		virtual LRESULT onPointerDown(
			WPARAM wParam,
			LPARAM lParam) {
			return DefWindowProc(
				hWnd, WM_POINTERDOWN, wParam, lParam);
		}
		virtual LRESULT onPointerEnter(
			WPARAM wParam,
			LPARAM lParam) {
			return DefWindowProc(
				hWnd, WM_POINTERENTER, wParam, lParam);
		}
		virtual LRESULT onPointerHWheel(
			WPARAM wParam,
			LPARAM lParam) {
			return DefWindowProc(
				hWnd, WM_POINTERHWHEEL, wParam, lParam);
		}
		virtual LRESULT onPointerLeave(
			WPARAM wParam,
			LPARAM lParam) {
			return DefWindowProc(
				hWnd, WM_POINTERLEAVE, wParam, lParam);
		}
		virtual LRESULT onPointerUp(
			WPARAM wParam,
			LPARAM lParam) {
			return DefWindowProc(
				hWnd, WM_POINTERUP, wParam, lParam);
		}
		virtual LRESULT onPointerUpdate(
			WPARAM wParam,
			LPARAM lParam) {
			return DefWindowProc(
				hWnd, WM_POINTERUPDATE, wParam, lParam);
		}
		virtual LRESULT onPointerWheel(
			WPARAM wParam,
			LPARAM lParam) {
			return DefWindowProc(
				hWnd, WM_POINTERWHEEL, wParam, lParam);
		}
		virtual LRESULT onPowerBroadcast(
			WPARAM wParam,
			LPARAM lParam) {
			return DefWindowProc(
				hWnd, WM_POWERBROADCAST, wParam, lParam);
		}
		virtual LRESULT onSysKeyDown(
			WPARAM wParam,
			LPARAM lParam) {
			return DefWindowProc(
				hWnd, WM_SYSKEYDOWN, wParam, lParam);
		}
		virtual LRESULT onSysKeyUp(
			WPARAM wParam,
			LPARAM lParam) {
			return DefWindowProc(
				hWnd, WM_SYSKEYUP, wParam, lParam);
		}
		virtual LRESULT onWindowPosChanged(
			WPARAM wParam,
			LPARAM lParam) {
			return DefWindowProc(
				hWnd, WM_WINDOWPOSCHANGED, wParam, lParam);
		}
		...
		// Window is a RAII class and cannot be moved nor
		// copied. To do so, use shared_ptr or unique_ptr.
		// Inherited classes will automatically have their
		// move/copy disabled, unless it is written explicitly,
		// without calling this base class move/copy.
		...
		UINT getDpi() const {
			return validateSystemCall(GetDpiForWindow(*this));
		}
