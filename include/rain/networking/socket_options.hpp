// These classes allow applying properties to an underlying
// Socket by inheriting them. All of these classes have
// constructors which should be called after that of a
// resource-owning Socket constructor.
//
// To ensure that nativeSocket() is overridden at
// construction, these classes must inherit a
// resource-owning Socket constructor. Multiple options may
// be set by stacking templates; since the class generated
// from one option template is a valid Socket for another
// option template.
#pragma once
...
namespace Rain::Networking {
	...
	class ReuseAddressSocketOption
		...
		class _ReuseAddressSocketOption {
			public:
			_ReuseAddressSocketOption(
				ReuseAddressSocketOption *that) {
				SocketOptionInterface::setOption(
					that->nativeSocket(),
					SOL_SOCKET,
					SO_REUSEADDR,
