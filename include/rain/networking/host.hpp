// Host, Node, Service implement representations of the
...
namespace Rain::Networking {
	...
	// node/service specification for an internet address.
	//
	// A collection of string node, service, with some helper
	// functions bundled in.
	class Host {
		public:
		// Empty node will be converted to nullptr when passed
		// to gai, binding to all available interfaces. Empty
		// service will be converted to nullptr when passed to
		// gai, binding to random port (port 0). Both cannot be
		// empty when used with gai.
		//
		// Usually, node is case-insensitive, but host allows
		// for storage of cased nodes. Comparison does not take
		// into account case.
		...
		std::string asStr() const {
			// Host:Service notation only contains a ':' if
			// Service is not empty, or if both are empty.
			return this->service.empty()
				? (this->node.empty() ? ":" : this->node)
		...
		bool operator==(Host const &other) const {
			...
			return strcasecmp(
							 this->node.c_str(), other.node.c_str()) ==
				0 &&
