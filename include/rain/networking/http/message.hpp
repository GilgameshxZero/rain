...
namespace Rain::Networking::Http {
	class MessageSpecInterface
		...
		class IdentityTransferEncodingIStreamBuf
			...
			// Content length, if available. If SIZE_MAX, then we
			// will read until sourceStreamBuf is invalidated.
			...
			// Disable copy.
			IdentityTransferEncodingIStreamBuf(
				IdentityTransferEncodingIStreamBuf const &) =
				delete;
			...
			protected:
			// Re-fill the buffer from the source, decrementing
			// contentLength if necessary.
			virtual int_type underflow() noexcept override {
				...
				if (this->gptr() == this->egptr()) {
					std::size_t cFilled{static_cast<std::size_t>(
						this->sourceStreamBuf->sgetn(
							&this->buffer[0],
							std::min(
								this->buffer.length(),
								this->contentLengthRemaining)))};
					this->contentLengthRemaining -=
						this->contentLengthRemaining == SIZE_MAX
						? 0
						: cFilled;

					// None filled, so we are done (or the Socket has
					// been closed/etc.).
					if (cFilled == 0) {
		...
		class ChunkedTransferEncodingIStreamBuf
			...
			// Bytes remaining in the current chunk. Can be zero,
			// in which case we process the next chunk.
			...
			virtual int_type underflow() noexcept override {
				// Only refill buffer if it has been exhausted.
				if (this->gptr() == this->egptr()) {
					// If chunkLenRemaining is 0, we need to interpret
					// it more.
					if (this->chunkLenRemaining == 0) {
						std::string chunkLenBuffer(20, '\0');
						std::istream chunkLenStream(
							this->sourceStreamBuf);
						chunkLenStream.getline(
							&chunkLenBuffer[0],
							chunkLenBuffer.length(),
							'\n');
					...
					// None filled, so the stream has probably been
					// closed.
					if (cFilled == 0) {
						return traits_type::eof();
					}

					// If chunkLenRemaining has just been set to 0,
					// ignore the \r\n coming right after.
					if (this->chunkLenRemaining == 0) {
	...
	class MessageSpec : public Message,
		...
		// Body is an uncopyable istream wrapping a custom
		// streambuf.
		Body body;

		private:
		// Dynamic storage for streambufs created for body
		// parsing.
		std::vector<std::unique_ptr<std::streambuf>>
			bodyTransferEncodingStreamBufs;

		public:
		// Constructor arguments passed in from R/R. Move
		// construct heavy arguments with rvalue references.
		MessageSpec(
			Headers &&headers = {},
			Body &&body = {},
			Version version = {})
		...
		virtual void ppEstimateContentLength(
			bool allowZero) override {
			std::vector<Header::TransferEncoding>
				transferEncoding = this->headers.transferEncoding();

			// Set Content-Length if not yet set and
			// Transfer-Encoding is identity and res.body's length
			// can be determined.
			if (
				...
				if (inAvail == -1 && allowZero) {
					...
					this->headers.contentLength(
						static_cast<std::size_t>(inAvail));
				}
			}
		}
		...
		virtual void ppDefaultContentType() override {
			if (
				this->headers.contentLength() > 0 &&
				this->headers.find("Content-Type") ==
					this->headers.end()) {
				this->headers.contentType(MediaType(
					MediaType::OCTET_STREAM, "charset=UTF-8"));
			}
		}
		...
		virtual void recvBody(std::istream &stream) override {
			// Create streambuf for body based on
			...
			// Throw on any unsupported transfer encodings or if
			// there are too many.
			...
			// For each transfer encoding, pass it through the
			...
			for (auto it = transferEncoding.rbegin();
				...
				switch (*it) {
					case Header::TransferEncoding::IDENTITY:
						...
						// content-length.
						this->bodyTransferEncodingStreamBufs
							.emplace_back(
								new IdentityTransferEncodingIStreamBuf(
									curStreamBuf,
									it == transferEncoding.rbegin()
										? contentLength
										: SIZE_MAX));
					...
					case Header::TransferEncoding::CHUNKED:
						this->bodyTransferEncodingStreamBufs
							.emplace_back(
								new ChunkedTransferEncodingIStreamBuf(
									curStreamBuf));
						break;
				...
				// The streambuf from the previously processed
				// encoding needs to be updated.
				curStreamBuf =
					this->bodyTransferEncodingStreamBufs.back().get();
			}
