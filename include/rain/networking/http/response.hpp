...
namespace Rain::Networking::Http {
	class ResponseMessageSpecInterface
		...
		class Version_0_9BodyIStreamBuf
			...
			// Leftover bytes from failed version parsing, to be
			// sent first.
			...
			// Disable copy.
			Version_0_9BodyIStreamBuf(
				Version_0_9BodyIStreamBuf const &) = delete;
			Version_0_9BodyIStreamBuf &operator=(
				Version_0_9BodyIStreamBuf const &) = delete;
	...
	class ResponseMessageSpec
		...
		virtual void recvWith(std::istream &stream) override {
			...
			try {
				stream.getline(
					&versionStr[0], versionStr.length(), ' ');

				// Create a copy of versionStr so that the original
				// remains unchanged, in case the parsing below
				// throws. Then, the version is 0.9, and we need to
				// use the bytes in versionStr again as the body.
				...
				// Otherwise, this throws, and we assume HTTP/0.9.
				this->version = Version(versionStrCopy.substr(5));
			} catch (...) {
				// Assume that we are HTTP/0.9.
				this->version = Version::_0_9;

				// The body contains the first gcount bytes in
				// versionStr. Regardless of whether failbit is set,
				// the final of those gcount bytes will be correct
				// from the space-init of versionStr.
				versionStr.resize(
					static_cast<std::size_t>(stream.gcount()));
				this->version_0_9BodyIStreamBuf.reset(
					new Version_0_9BodyIStreamBuf(
						stream.rdbuf(), versionStr));
				this->body =
					Body(this->version_0_9BodyIStreamBuf.get());

				return;
			}

			// Version successfully parsed (and dealt with if
			// 0.9). Throw if still unexpected.
			switch (this->version) {
				case Version::_0_9:
			...
			// We must be in version 1.0 or 1.1. Get the rest of
			// the startline.
			...
			try {
				...
				stream.getline(
					&statusCodeStr[0], statusCodeStr.length(), ' ');
				statusCodeStr.resize(
					static_cast<std::size_t>(std::max(
						std::streamsize(0), stream.gcount() - 1)));
			...
			// Similar to request parsing, the body termination is
			// determined by a combination of Content-Length and
			// Transfer-Encoding.
			try {
