...
namespace Rain::Math {
	// First argument should be a sidegraded type, and the
	// second argument should be a Functional::TypeUpgraded
	// std::size_t.
	template<
	...
	class TensorInterface {
			...
			std::string message(int error) const noexcept {
				switch (error) {
					...
					case Error::OUT_OF_RANGE:
						return "Requested index was out of range.";
					case Error::INVALID_PERMUTATION:
						return "Provided arguments did not form a "
									 "valid permutation.";
					default:
						return "Generic.";
				}
		...
		static inline void applyOver(
			...
			for (
				std::size_t i{0};
				i < result.SIZE[result.TRANSPOSE[0]];
				i++) {
	...
	class TensorType : public TensorInterface {
		public:
		...
		// Allow access from TensorInterface.
		...
		std::array<std::size_t, ORDER> SIZE, SIZE_UNDERLYING;
		...
		std::ostream &streamOutPadded(
			...
			for (std::size_t i{1}; i < this->SIZE[0]; i++) {
		...
		std::ostream &streamOutPadded(
			...
			if (this->SIZE[this->TRANSPOSE[0]] == 0) {
		...
		TensorType(
			...
			VALUES{values},
			RANGES{ranges},
			SIZE{TensorInterface::calcSizes(ranges)},
			SIZE_UNDERLYING{sizesUnderlying},
			OFFSET{offset},
			TRANSPOSE{dimPerm} {}
		...
		TensorType(
			...
			VALUES{new Value[TensorInterface::calcSizesProduct(
				sizes)]{}},
			RANGES{TensorInterface::makeRangesDefault(sizes)},
			SIZE{sizes},
			SIZE_UNDERLYING{sizes},
			OFFSET{0},
			TRANSPOSE{
				TensorInterface::makeDimPermDefault<ORDER>()} {}
		...
		TensorType(
			...
			Values &&...values) :
			VALUES{
				new Value[TensorInterface::calcSizesProduct(sizes)]{
		...
		TensorType(TypeThis const &other) :
			...
			RANGES{other.RANGES},
			SIZE{other.SIZE},
			SIZE_UNDERLYING{other.SIZE_UNDERLYING},
			OFFSET{other.OFFSET},
			TRANSPOSE{other.TRANSPOSE} {}
		// The default constructor generates a Tensor
		// with only a single element.
		TensorType() :
			TensorType(TensorInterface::makeOnesSizes<ORDER>()) {}
		...
		auto &operator=(
			...
			this->SIZE_UNDERLYING = other.SIZE_UNDERLYING;
		...
		// or not.
		//
		// It is always constructed without specifying SIZE,
		...
		inline Value const &operator[](std::size_t idx) const {
			if constexpr (Platform::isDebug()) {
				if (idx >= this->SIZE[this->TRANSPOSE[0]]) {
		...
		Tensor<Value, ORDER - 1> const operator[](
			...
			for (std::size_t i{1}; i < ORDER; i++) {
				sizesUnderlying[newDimPerm[i - 1]] =
					this->SIZE_UNDERLYING[this->TRANSPOSE[i]];
				if (this->TRANSPOSE[i] > this->TRANSPOSE[0]) {
					...
					ranges[newDimPerm[i - 1]].step =
						...
						this->SIZE_UNDERLYING[this->TRANSPOSE[0]];
		...
		inline auto asApplyOver(
			...
			result.template applyOver<REMAINING_ORDER>(
				std::forward<decltype(callable)>(callable),
				std::forward<decltype(others)>(others)...);
			return result;
		}
		...
		inline auto accumulate(
			auto &&callable,
			ResultType accumulator = ResultType{}) const {
			this->applyOver<0>(
				[&accumulator, &callable](Value const &value) {
					callable(accumulator, value);
				});
			return accumulator;
		}
		...
		inline auto exp() {
			this->applyOver<0>(
				[](Value &left) { left = std::exp(left); });
			return *this;
		}
		inline auto asExp() const {
			auto result{this->copy()};
			return result.exp();
		}
		inline auto log() {
			this->applyOver<0>(
				[](Value &left) { left = std::log(left); });
			return *this;
		}
		inline auto asLog() const {
			auto result{this->copy()};
			return result.log();
		}
		inline auto clamp(
			...
			Value const &high =
				std::numeric_limits<Value>::max()) {
			this->applyOver<0>([&low, &high](Value &left) {
				left = std::clamp(left, low, high);
			});
			return *this;
		}
		inline auto asClamp(
		...
		bool isClean() const {
			...
			for (std::size_t i{0}; i < ORDER; i++) {
				if (
					...
					this->RANGES[i].stop != this->SIZE[i] ||
		...
		// Property functions.
		//
		// Transposition does not change `SIZE`, and so some
		// functions should use `SIZE`, while others should use
		// `size()`.
		...
		inline auto sizeProduct() const {
			return TensorInterface::calcSizesProduct(this->SIZE);
		}
			...
			if constexpr (Platform::isDebug()) {
				...
				for (std::size_t i{0}; i < ORDER; i++) {
					if (dimPermSorted[i] != i) {
						throw Exception(Error::INVALID_PERMUTATION);
					}
				}
			}
		...
		auto asReshape(
			...
			std::size_t toInfer{NEW_ORDER}, newTotalSize{1},
				totalSize{
					TensorInterface::calcSizesProduct(this->SIZE)};
		...
		// Shorthand for changing underlying type via
		...
		inline auto asRetype() const {
			...
			result.template applyOver<0>(
				[](OtherValue &left, Value const &right) {
					left = static_cast<OtherValue>(right);
				},
				*this);
			return result;
		}
		...
		auto asMultiply(
			...
			if constexpr (Platform::isDebug()) {
				for (std::size_t i{0}; i < CONTRACT_ORDER; i++) {
					if (
						this->SIZE[this
								->TRANSPOSE[thisContractDims[i]]] !=
						other.SIZE[other
								.TRANSPOSE[otherContractDims[i]]]) {
						throw Exception(Error::SIZES_MISMATCH);
					}
				}
			}
			...
			for (
				std::size_t i{0}, j{CONTRACT_ORDER};
				i < OTHER_ORDER;
				i++) {
			...
			for (
				std::size_t i{0}; i < ORDER - CONTRACT_ORDER; i++) {
				resultSize[i] = thisSize[i];
			}
			for (
				std::size_t i{0}; i < OTHER_ORDER - CONTRACT_ORDER;
				i++) {
		...
		// Allow access from stream operators.
		//
