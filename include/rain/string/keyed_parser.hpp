// A bundle of parsers, one for each key. When parsing, pass
// a key + a string value to parse; the previously set
// parser will parse the string value into the parsing store
// set earlier.
#pragma once
...
namespace Rain::String {
	// A bundle of parsers, one defined for each key passed
	// in. Parsing a key-value pair will attempt to use the
	// pre-defined parser for that key to parse the value.
	// Like any unchecked string parser, ill-formed strings
	...
	class KeyedParser {
		...
		class ErrorCategory : public std::error_category {
			...
			char const *name() const noexcept {
				return "Rain::String::KeyedParser";
			}
		...
		// Parser signature. The argument stores the string to
		// be parsed. The function implicitly knows where to
		// store the result.
		...
		bool addParser(std::string const &key, Store &store) {
			return !this->parsers
								.insert(
									std::make_pair(
										key, this->getParser(store)))
								.second;
		}

		// Parse a text given its key. May throw exceptions.
		// Returns true on parser error.
		bool parse(
		...
		private:
		// Overloads for getParser which return a value parser
		// based on the type to store into. Bools only parse to
		// true if the string is some variation of "true" or a
		// non-zero number.
		//
		...
		Parser getParser(
			unsigned long long &store) const noexcept {
			return Parser([&store](std::string const &value) {
		...
		Parser getParser(
			...
			return Parser(
				[this, &store](std::string const &value) {
					store.push_back(Inner());
					this->getParser(store.back())(value);
					return false;
				});
		}
		// std::size_t parser not provided without
		// std::enable_if. Use one of the existing numerical
		// types instead.
	};
}
