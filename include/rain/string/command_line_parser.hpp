...
namespace Rain::String {
	...
	class CommandLineParser : public KeyedParser {
		...
		// Parse a list of command-line arguments. Do not pass
		// the first argument (typically filename) into this
		// parser. Command-line arguments come in key-value
		// pairs. Keys longer than 1 character must be prefixed
		// with --. Values must come after keys, but optionally
		// connected with the key with a
		// =. Short keys must be 1 character long and prefixed
		// with -, with their values optionally coming
		// immediately after the key. Returns true on any parser
		// error. Throws on other errors.
		bool parse(
			int argc,
			...
			for (int i{0}; i < argc; i++) {
				...
				if (arg[0] != '-') {
					...
					// Otherwise, this argument does not have a key.
					...
					if (*value == '\0') {
						...
						// argument. Copy key name out.
						keyName = std::string(arg + 2, value - arg - 2);

						// If next argument begins with - and is a key,
						// then this key is a flag (without value).
						// Otherwise, next argument is the value for
						// this key.
						value = (i + 1 == argc || argv[i + 1][0] == '-')
							? nullptr
							: argv[++i];
					} else {
						// An explicit = delimiter exists, and the value
						// is after that.
						keyName =
							std::string(arg + 2, value++ - arg - 2);
					}
				} else if (arg[1] == '\0') {
					...
					if (arg[2] == '\0') {
						// Argument ends, so value is next argument.
						// Again, check if the key is a flag.
						value = (i + 1 == argc || argv[i + 1][0] == '-')
							? nullptr
							: argv[++i];
					} else {
				...
				// Flags are explicitly parsed as TRUE and should be
				// set by the caller as bool stores.
				if (value == nullptr) {
					value = "1";
				}
