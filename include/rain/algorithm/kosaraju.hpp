...
namespace Rain::Algorithm {
	...
	// condensation graph of G. Kosaraju's algorithm provides
	// the SCCs in topologically sorted order, but is a
	// constant factor slower than Tarjan’s.
	//
	// Returns the number of CCs, and the 0-indexed index of
	// the SCC that each vertex belongs to. SCC indices are
	...
	sccKosaraju(
		...
		for (std::size_t i{0}; i < vPostOrder.size(); i++) {
			if (
				...
				std::numeric_limits<std::size_t>::max()) {
				subroutineSecond(
					vPostOrder[vPostOrder.size() - 1 - i]);
				cScc++;
			}
		}
