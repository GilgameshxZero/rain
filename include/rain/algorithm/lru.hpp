// Least-recently-used cache implemented with a linked list
// + hashmap. O(1) average access. Not thread-safe.
#pragma once
...
namespace Rain::Algorithm {
	// Least-recently-used cache implemented with a linked
	// list + hashmap. O(1) average access. Not thread-safe.
	//
	// Value will be moved constructed if passed as
	...
	class LruCache {
		...
		using InternalHashMap = std::unordered_map<
			Key,
			typename InternalList::iterator,
			Hash,
			KeyEqual>;

		// List stores key/value pairs in LRU order; hashMap
		// looks up list iterators based on key. Key reference
		// in list points to key owned by hashMap.
		...
		bool empty() const noexcept {
			return this->lruList.empty();
		}
		auto begin() const noexcept {
			return this->lruList.begin();
		}
		auto end() const noexcept {
			return this->lruList.end();
		}
		auto begin() noexcept { return this->lruList.begin(); }
		auto end() noexcept { return this->lruList.end(); }

		// Returns an iterator to the hashmap object if
		// available, otherwise this->end(). Updates LRU
		// ordering.
		...
		// pair. In either case, move the pair to the front of
		// the cache.
		//
		...
		std::pair<typename InternalList::iterator, bool>
			...
			if (findIt != this->hashMap.end()) {
				// If key exists, simply move it to the front of the
				// list.
				this->lruList.splice(
					this->lruList.begin(),
					this->lruList,
				...
				// Move-assign value in; key does not need to be
				// changed.
			...
			// Insert into hashMap first, with default value
			// iterator.
			typename InternalHashMap::iterator mapIt;
			std::tie(mapIt, std::ignore) = this->hashMap.emplace(
				std::forward<KeyType>(key), this->lruList.end());

			// Insert value with key reference.
			this->lruList.emplace_front(
				mapIt->first, std::forward<ValueType>(value));
