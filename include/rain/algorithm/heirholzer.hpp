...
namespace Rain::Algorithm {
	// Constructs an Eulerian cycle/circuit with Hierholzer's
	// in O(E). The provided graph must be unweighted and
	// simple.
	//
	// Returns a list of points on the cycle. If the cycle is
	// not constructable, returns an empty list.
	inline std::vector<std::size_t> eulerCycleHeirholzer(
		std::vector<std::unordered_multiset<std::size_t>> const
			&edges,
		bool directed) {
		...
		while (cycle.size() < target) {
			...
			do {
				...
				residual[cycle.back()].erase(
					residual[cycle.back()].begin());
				if (!directed) {
					residual[current].erase(
						residual[current].find(cycle.back()));
				}
