// A special optimization of the segment tree without
// support for lazy propagation. Only support point updates
// instead of range updates, but has a much faster constant
// factor.
#pragma once
...
namespace Rain::Algorithm {
	// Segment tree without lazy propagation nor range
	// updates.
	//
	// Implements the policy pattern. Policy must implement
	// the following interface (omitted from being an actual
	// interface, as static virtual functions are not
	// supported):
	//
	// static constexpr Value DEFAULT_VALUE: Identity values
	// at creation and aggregation.
	//
	// static void apply(Value &value, Update const &update):
	// Fully apply an update to a leaf node.
	//
	// static Result aggregate(Result const &left, Result
	// const &right): Aggregate two results from queries on
	// children. Aggregating with a Result converted from a
	// default Value should do nothing.
	//
	// static void retrace(Value &value, Value const &left,
			...
			for (left += this->values.size() / 2,
				...
				if (left % 2 == 1) {
					resLeft = Policy::aggregate(
						resLeft, this->values[left++]);
				}
				if (right % 2 == 1) {
					resRight = Policy::aggregate(
						this->values[--right], resRight);
				}
			}
