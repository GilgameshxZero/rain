...
namespace Rain::Algorithm {
	// Computes strongly connected components (SCCs) for a
	// simple graph G in O(V+E). The SCCs form an acyclic
	// condensation graph of G. Typically, Tarjan's is more
	// efficient than Kosarju's algorithm for SCCs, though
	// Kosarju's algorithm provides the SCCs in topologically
	// sorted order (and Tarjan's in reverse topological
	// order).
	//
	...
	sccTarjan(
		...
		// Tarjan’s requires a subroutine for its inner DFS,
		// which we implement via recursive lambda with an
		// explicit return type.
		auto subroutine{[&](std::size_t _i) {
			auto subroutineInner{
				[&](std::size_t i, auto &subroutineRef) -> void {
					...
					for (auto const &j : edges[i]) {
						if (
							...
							lowLink[i] =
								std::min(lowLink[i], preOrderId[j]);
						}
					}
