...
namespace Rain::Algorithm {
	// Computes SSSP on a weighted, directed graph with SPFA
	// in O(NM). The provided graph must be simple and may
	// contain negative weights.
	//
	// SPFA is a variation of Bellman-Ford which tracks
	// vertices which need to be considered for relaxation.
	// Any vertices which are relaxed after the `N-1`-th
	// iteration must be reachable via a negative-weight
	// cycle.
	//
	// Returns a list of minimal distances to each vertex, and
	// the penultimate vertex on the shortest path to each
	// vertex. If a vertex is unreachable, the distance is
