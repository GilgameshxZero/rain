// Knuth-Morris-Pratt O(N+M) single perfect string-matching
// algorithm.
#pragma once
...
namespace Rain::Algorithm {
	// Compute partial match table (also known as the failure
	// function) (used in KMP) for a string.
	//
	// The partial match table specifies where to "rewind" the
	// matching process to if it failes on a given character.
	inline std::vector<std::size_t> computeKmpPartialMatch(
		...
		// This represents -1, i.e. we can resume matching for
		// the first character of the string at the next
		// position in the text.
		partialMatch[0] = SIZE_MAX;

		// How far into search string s we must begin (to our
		// best knowledge) if we mismatch.
	...
	// KMP search for needle in haystack, in O(N+M).
	// Optionally pass a candidate parameter if want to
	// continue from a previous search. Returns a pair of
	// (match, candidate). The match is nullptr if no match
	// found, otherwise returns a char * to the first
	// character of the first match found. The candidate is
	// the updated candidate at the termination of the search.
	inline std::pair<char *, std::size_t> kmpSearch(
		...
		std::size_t candidate = 0) {
		// Use these variables, which are set based on whether
		// or not default partialMatch and candidate were passed
		// in.
		std::vector<std::size_t> const &partialMatchProxy =
			partialMatch.size() == 0
			? computeKmpPartialMatch(needle)
			: partialMatch;
		...
		for (std::size_t a{0}; a < haystackLen;) {
			if (haystack[a] == needle[candidate]) {
				...
				// Use the partial match table to resume if a match
				// fails.
	...
	// The first element of returned pair will be out of range
	// if not found.
	inline std::pair<std::size_t, std::size_t> kmpSearch(
		...
		std::pair<char *, std::size_t> searchRes = kmpSearch(
			...
			candidate);
		return {
			searchRes.first - haystack.c_str(), searchRes.second};
	}
	inline std::pair<std::size_t, std::size_t> kmpSearch(
		std::string const &haystack,
		std::string const &needle,
