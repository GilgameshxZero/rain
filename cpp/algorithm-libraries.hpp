...
namespace Rain {
	...
	unsigned long long fastModExp(unsigned long long base,
	...
	std::size_t rabinKarpMatch(const std::string &text,
			std::string pattern,
			unsigned long long prime = 1610612741);
...
namespace Rain {
	...
	std::size_t rabinKarpMatch(const std::string &text,
		...
		unsigned long long curHash = 0, targetHash = 0,
											 basePower = fastModExp(
													 alphabetSize, pattern.length() - 1, prime);
		...
		for (std::size_t a = 0; a < text.length(); a++) {
			...
			if (a >= pattern.length())
				curHash =
						(prime + curHash -
								(basePower * (text[a - pattern.length()] + 128LL)) % prime) %
						prime;
			curHash = ((curHash * alphabetSize) + (text[a] + 128LL)) % prime;

			if (curHash == targetHash &&
					text.substr(a - pattern.length() + 1, pattern.length()) ==
							pattern)	// check if string actually matches
