...
When in doubt: directories are specified with an ending '/'. Sometimes functions
may not work if this format is not followed. Assume functions don't work with
unicode/UTF8 multibyte strings, unless specified.
...
namespace Rain {
	// turn a string into LPCWSTR with a "\\?\" prefix, for use in long paths
	// unicode functions on Windows
	...
	unsigned short statStMode(std::string path) {
		struct _stat buffer;
		_stat(path.c_str(), &buffer);
		return buffer.st_mode & S_IFMT;
	}
	bool fileExists(std::string path) { return statStMode(path) == S_IFREG; }
	bool dirExists(std::string path) { return statStMode(path) == S_IFDIR; }
	...
	// returns current directory of the executable (not the same as the path to
	// the executable, sometimes)
	std::string getWorkingDirectory();

	// works with Unicode/UTF8-multibyte
	std::string getExePath();

	// converts a path to a file into either the filename or the directory for
	// that file
	std::string getPathDir(std::string path);
	std::string getPathFile(std::string path);

	// Get all files or all directories in a directory, in a certain format. NOTE:
	// Takes and returns UTF8 multibyte strings - but works with unicode
	// directories.
	std::vector<std::string> getFiles(std::string directory,
			std::string format = "*");
	std::vector<std::string> getDirs(std::string directory,
			std::string format = "*");
	...
	std::vector<std::string> getFilesRec(std::string directory,
			std::string format = "*",
			std::set<std::string> *ignore = NULL,
			std::set<std::string> *want = NULL);
	std::vector<std::string> getDirsRec(std::string directory,
			std::string format = "*",
			std::set<std::string> *ignore = NULL,
			std::set<std::string> *want = NULL);
	...
	// removes all directories and files under a directory, but not the directory
	// itself works with unicode/multibyte UTF8 second argument specifies
	// files/directories to not remove; use \ end to specify dir use absolute
	// paths in ignore
	void rmDirRec(std::string dir,
			std::set<std::string> *ignore = NULL,
			std::set<std::string> *want = NULL);

	// copies directory structure over, replacing any files in the destination,
	// but not deleting any unrelated files
	void cpyDirRec(std::string src,
			std::string dst,
			std::set<std::string> *ignore = NULL);

	// allows for retrieval of file information and testing if two paths point to
	// the same file
...
namespace Rain {
	...
	std::vector<std::string> getFiles(std::string directory, std::string format) {
		...
		HANDLE hFind =
				::FindFirstFileW(pathToLongPath(std::wstring(unicodesp)).c_str(), &fd);
		...
		if (hFind != INVALID_HANDLE_VALUE) {
			do {
				if (!(fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) &&
					...
					WideCharToMultiByte(
							CP_UTF8, 0, fd.cFileName, -1, multibyte, 32767, NULL, NULL);
	...
	std::vector<std::string> getFilesRec(std::string directory,
		...
		for (std::size_t a = 0; a < files.size(); a++) {
			if (ignore == NULL ||
					ignore->find(pathToAbsolute(directory + files[a])) == ignore->end()) {
				if (want == NULL ||
						want->find(pathToAbsolute(directory + files[a])) != want->end()) {
		...
		for (std::size_t a = 2; a < dirs.size(); a++) {
			if (ignore != NULL && ignore->find(pathToAbsolute(
																directory + dirs[a] + "\\")) != ignore->end())
				continue;

			std::vector<std::string> subrel;
			if (want == NULL || want->find(pathToAbsolute(
															directory + dirs[a] + "\\")) != want->end()) {
	...
	void rmDirRec(std::string dir,
		...
		for (std::size_t a = 0; a < lfile.size(); a++) {
			MultiByteToWideChar(CP_UTF8, 0, pathToAbsolute(dir + lfile[a]).c_str(),
					-1, unicode, 32767);
			if (ignore == NULL ||
					ignore->find(pathToAbsolute(dir + lfile[a])) == ignore->end()) {
				if (want == NULL ||
						want->find(pathToAbsolute(dir + lfile[a])) != want->end()) {
		...
		for (std::size_t a = 2; a < ldir.size(); a++)	// skip . and ..
		{
			MultiByteToWideChar(CP_UTF8, 0,
					pathToAbsolute(dir + ldir[a] + '\\').c_str(), -1, unicode, 32767);
			if (ignore == NULL ||
					ignore->find(pathToAbsolute(dir + ldir[a] + '\\')) == ignore->end()) {
				if (want == NULL ||
						want->find(pathToAbsolute(dir + ldir[a] + '\\')) != want->end()) {
					// want everything under this directory, so don't pass any argument
					// forward
					rmDirRec(dir + ldir[a] + '\\', ignore, NULL);

					// RemoveDirectory removes symbolic links even when not empty, so only
					// call if the directory really is empty
	...
	void cpyDirRec(std::string src,
		...
		for (std::string s : filesRel) {
			...
			CopyFileW(pathToLongPath(src + s).c_str(),
					pathToLongPath(dst + s).c_str(), FALSE);
		}
	}

	BY_HANDLE_FILE_INFORMATION getFileInformation(std::string path) {
		HANDLE handle = CreateFileW(pathToLongPath(pathToAbsolute(path)).c_str(), 0,
	...
	FILETIME getLastModTime(std::string path) {
		...
		HANDLE hfile = CreateFileW(unicode, GENERIC_READ, FILE_SHARE_READ, NULL,
				OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
