...
namespace Rain {
	...
	public:
		...
		bool wait_for(std::unique_lock<std::mutex>& lck,
			...
			return std::condition_variable::wait_for(lck, rel_time, std::bind(&ConditionVariable::predicate, this));
		}

		template <class Clock, class Duration>
		bool wait_until(std::unique_lock<std::mutex>& lck,
			const std::chrono::time_point<Clock, Duration>& abs_time) {
			return std::condition_variable::wait_for(lck, abs_time, std::bind(&ConditionVariable::predicate, this));
		}
