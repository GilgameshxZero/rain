...
namespace Rain {
	...
	class RainWindow {
	 public:
		...
		int create(std::unordered_map<UINT, MSGFC>
									 *msgm,	// pointer to a map which maps messages to functions
													 // to be called when the message is received
				MSGFC *intfc =
						NULL,	// interceptor function: if the application wants to define a
									 // custom message handler, here is where to do it
				UINT style = 0,
				int cbClsExtra = 0,
				int cbWndExtra = 0,
		...
		// can be called by anything; enter message loop for all windows; returns
		// when WM_QUIT is received
