...
class Window : public Rain::Windows::Window {
	...
	std::vector<Rain::Algorithm::Geometry::PointL>
		clickedPoints;

	protected:
	LRESULT onCreate(WPARAM, LPARAM) override {
		Rain::Windows::validateSystemCall(
			RegisterTouchWindow(*this, NULL));
	...
	LRESULT onPaint(WPARAM wParam, LPARAM lParam) override {
		...
		auto dc{ps.hDc()};
		// Fails compilation as expected since copy is deleted
		...
		rectangle.right =
			(rcPaint.left * 2 + rcPaint.right) / 3;
		dc.fillRect(rectangle, {0x00ff0000});
		rectangle.left = rectangle.right;
		rectangle.right =
			(rcPaint.left + rcPaint.right * 2) / 3;
		...
		for (auto const &i : this->clickedPoints) {
			dc.ellipse(
				{i.x - SIZE, i.y - SIZE, i.x + SIZE, i.y + SIZE});
		}
...
int main() {
	...
	std::thread([&]() {
		...
		input.mi.dy = MulDiv(
			rect.top + 100, 65536, GetSystemMetrics(SM_CYSCREEN));
		input.mi.time = 0;
